...
import API from "../../services/api"; // ✅ ดึงข้อมูลจาก backend

const TempleEventCalendar = () => {
  const [currentDate, setCurrentDate] = useState(new Date());
  const [selectedDate, setSelectedDate] = useState(null);
  const [events, setEvents] = useState({}); // ✅ เก็บข้อมูลจาก backend

  // โหลดกิจกรรมจาก API
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        const res = await API.get("/events"); // backend return: [{id, title, start_time, end_time, type, participants, location}]
        ...
        res.data.forEach(event => {
          ...
          grouped[dateKey].push({
            ...
            participants: event.participants || "ทุกท่าน",
            location: event.location || "วัดกำแพง"
          });
        });
        setEvents(grouped);
      } catch (err) {
    ...
    <LayoutHome fullWidth>
      ...
      <section className="relative h-100 w-full">
        ...
        <div className="relative z-10 flex items-center justify-center h-[500px] text-center text-white">
          <div>
            <h1 className="text-4xl font-bold mb-4 text-orange-400">ปฏิทินกิจกรรมวัดกำแพง</h1>
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2">
              <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
                ...
                <div className="bg-gradient-to-r from-orange-500 to-amber-500 text-white p-6 flex items-center justify-between">
                  <button onClick={() => navigateMonth(-1)} className="p-2 rounded-full hover:bg-white/20 transition-colors">
                    <ChevronLeft className="w-6 h-6" />
                  </button>
                  <h2 className="text-2xl font-bold">{monthNames[month]} {year + 543}</h2>
                  <button onClick={() => navigateMonth(1)} className="p-2 rounded-full hover:bg-white/20 transition-colors">
                  ...
                  <div className="grid grid-cols-7 gap-1">
                    {days.map((day, index) => {
                      ...
                      return (
                        <div
                          key={day}
                          className={`h-24 p-2 border border-gray-100 rounded-lg cursor-pointer transition-all hover:shadow-md ${hasEvents ? 'bg-orange-50 border-orange-200' : 'hover:bg-gray-50'}`}
                        ...
                        >
                          ...
                          {hasEvents && (
                            <div className="text-xs">
                              ...
                              <div className="text-orange-600 truncate">{dayEvents[0].title}</div>
                              {dayEvents.length > 1 && (
            ...
            <div className="space-y-6">
                    ...
                    {events[selectedDate].map((event) => (
                      <div key={event.id} className={`p-4 rounded-lg border-2 ${getEventTypeColor(event.type)}`}>
                        ...
                        <div className="space-y-2 text-sm">
                          ...
                          <div className="flex items-center gap-2"><Users className="w-4 h-4" /><span>{event.participants}</span></div>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="bg-white rounded-2xl shadow-lg p-4">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex items-center gap-2"><div className="w-3 h-3 bg-green-500 rounded-full"></div><span>กิจกรรมประจำวัน</span></div>
                  <div className="flex items-center gap-2"><div className="w-3 h-3 bg-blue-500 rounded-full"></div><span>พิธีกรรมทางศาสนา</span></div>
                  <div className="flex items-center gap-2"><div className="w-3 h-3 bg-purple-500 rounded-full"></div><span>พิธีสำคัญ</span></div>
                  <div className="flex items-center gap-2"><div className="w-3 h-3 bg-orange-500 rounded-full"></div><span>วันพระ/งานพิเศษ</span></div>
                  <div className="flex items-center gap-2"><div className="w-3 h-3 bg-amber-500 rounded-full"></div><span>การบริจาค/ทำบุญ</span></div>
                </div>
              </div>
